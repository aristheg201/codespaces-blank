...
interface State {
  ...
  setModPolicy(path: string, patch: Partial<ModPolicy>): Promise<void>;
  redetectMod(path?: string): Promise<void>;
...
const EMPTY: WorkspaceSnapshot = { root: null, files: [], staged: [], scanning: false, audit: { total: 0, client: 0, server: 0, both: 0, unknown: 0, disabled: 0, android: 0, warnings: 0 } };
...
export const useManager = create<State>((set) => ({
  ...
  setModPolicy: async (filePath, patch) => { await guarded(set, async () => set({ snapshot: await window.bestiary.setModPolicy(filePath, patch) })); },
  redetectMod: async (filePath) => { await guarded(set, async () => set({ snapshot: await window.bestiary.redetectMod(filePath) })); },
