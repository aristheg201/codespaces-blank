...
$source = $source.Replace('5.1.3', '5.2.1').Replace('build-output-v513', 'build-output-v521')
...
$patches = $builderCopy + "`n" +
  ...
  "python 'bestiary-v521/patch_home.py'`nif (`$LASTEXITCODE -ne 0) { throw 'Unable to apply prominent mod manager entry.' }"
...
$homeStyle = Get-Content 'source/src/renderer/src/components/Home.css' -Raw
...
if ($homeSource -notmatch 'QUẢN LÝ MOD' -or $homeSource -notmatch 'MỞ QUẢN LÝ' -or $homeSource -notmatch 'bestiary-mod-manager') { throw 'Prominent mod manager Home entry missing.' }
if ($homeStyle -notmatch '\.bestiary-mod-manager') { throw 'Prominent mod manager styling missing.' }
if ($appSource -notmatch "currentVersion: '5.2.1'") { throw 'Launcher 5.2.1 updater version missing.' }
...
Write-Host 'Launcher 5.2.1 Content screen, prominent mod manager entry, updater, and profile contracts verified.'
