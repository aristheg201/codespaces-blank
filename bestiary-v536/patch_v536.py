...
# Electron must never remain as a headless zombie after the last Windows window is closed.
# Keep one process only and make a second launch restore/focus the existing BrowserWindow.
...
# A stale BrowserWindow reference is expected after a user closes the old window. Recreate it.
...
# If the custom title bar has a close IPC, it must quit rather than hide the only window.
# Support both the historical one-line form and a block form without touching minimize/maximize.
s = re.sub(
...
# Some older builds implemented close-to-background directly on BrowserWindow. Remove only the
# hide/preventDefault zombie pattern. Do not touch unrelated close handlers.
...
# Lifecycle events are idempotent and intentionally Windows-strict.
lifecycle = '''
app.on('before-quit', () => {
  isQuitting = true;
});
...
# Fail hard if any close-to-hide zombie pattern survived in the main process.
...
# Version-only renderer change. The 5.3.3 UI freeze remains intact.
