...
import type { AccountMode, AppUpdateState, AuthStatusEvent, GameLogEvent, LauncherSettings, LibraryKind, RendererApi, SkinSetRequest, UiProgressEvent } from '../shared/ipc';
...
const api: RendererApi = {
  ...
  getAccounts: () => ipcRenderer.invoke('bestiary:account-get'),
  setAccountMode: (mode: AccountMode) => ipcRenderer.invoke('bestiary:account-mode', mode),
  loginMicrosoft: () => ipcRenderer.invoke('bestiary:account-login-microsoft'),
  logoutMicrosoft: () => ipcRenderer.invoke('bestiary:account-logout-microsoft'),
  chooseSkinFile: () => ipcRenderer.invoke('bestiary:skin-choose'),
  setSkin: (request: SkinSetRequest) => ipcRenderer.invoke('bestiary:skin-set', request),
  resetSkin: () => ipcRenderer.invoke('bestiary:skin-reset'),
  onAuthStatus: (listener: (state: AuthStatusEvent) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, payload: AuthStatusEvent) => listener(payload);
    ipcRenderer.on('bestiary:auth-status', handler);
    return () => ipcRenderer.removeListener('bestiary:auth-status', handler);
  },
